...
export function buildSeedPrompt(input: SeedPromptInput): string {
  const sections = [
    ...
    markdownSection(
      ...
      `You are working on task ${input.task.key}. Use the bb tasks-plus CLI: comment substantive updates (bb tasks-plus comment ${input.task.key} --body ...), attach result artifacts, set status when done (bb tasks-plus update ${input.task.key} --status in_review) or explain blockage in a comment. Your thread is already attached to the task.`,
    ),
  ];

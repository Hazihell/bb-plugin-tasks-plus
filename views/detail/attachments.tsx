...
export function attachmentDownloadUrl(attachmentId: string): string {
  return `/api/v1/plugins/tasks-plus/http/attachments/download?attachmentId=${encodeURIComponent(attachmentId)}`;
}
...
function pluginToken(): Promise<string> {
  tokenPromise ??= (async () => {
    const response = await fetch("/api/v1/plugins/tasks-plus/token", {
      method: "POST",
...
export async function uploadAttachment(
  ...
  const response = await fetch(
    `/api/v1/plugins/tasks-plus/http/attachments/upload?${query.toString()}`,
    {
      method: "POST",

...
export default defineConfig({
  resolve: {
    alias: {
      // The `@/` path alias declared in tsconfig.json, for vendored shared UI.
  ...
  test: {
    ...
    // Testing-library's default 1s findBy*/waitFor timeout flakes on the
    // heavier UI suites (pager, manage, rail).
    testTimeout: 20_000,
